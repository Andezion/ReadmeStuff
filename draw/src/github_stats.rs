...
<text x="25" y="30" font-family="{font}" font-size="14" font-weight="600" fill="{title}">{dname}</text>
<text x="25" y="47" font-family="{font}" font-size="11" fill="{text2}">@{login}</text>
<g transform="translate(456,35)">
  <circle r="22" fill="{bg}" stroke="{rcolor}" stroke-width="2.5"/>
  <text text-anchor="middle" dy="6" font-family="{font}" font-size="15" font-weight="700" fill="{rcolor}">{rank}</text>
</g>
