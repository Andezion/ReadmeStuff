...
pub fn render_github_contributions(w: &GithubContributionsWidget, theme: Theme) -> String {
    ...
    for (i, (label, value)) in row1.iter().enumerate() {
        ...
        stat_svg.push_str(&format!(
            "<text x=\"{x}\" y=\"90\" font-family=\"{font}\" font-size=\"18\" font-weight=\"700\" fill=\"{tv}\">{value}</text>\
             <text x=\"{x}\" y=\"106\" font-family=\"{font}\" font-size=\"11\" fill=\"{tl}\">{label}</text>",
    ...
    for (i, (label, value)) in row2.iter().enumerate() {
        let x = col_x[i];
        stat_svg.push_str(&format!(
            "<text x=\"{x}\" y=\"136\" font-family=\"{font}\" font-size=\"18\" font-weight=\"700\" fill=\"{tv}\">{value}</text>\
             <text x=\"{x}\" y=\"150\" font-family=\"{font}\" font-size=\"11\" fill=\"{tl}\">{label}</text>",
            tv = c.text_primary, tl = c.text_secondary,
        ));
    }
...
<text x="25" y="35" font-family="{font}" font-size="14" font-weight="600" fill="{title}">Contributions (This Year)</text>
