...
fn extract_widget(svg: &str) -> Option<(String, String)> {
    ...
    let content_start = svg[after_clip_g..]
        .find("/>")
        .map(|p| p + after_clip_g + 2)?;
...
fn namespace_rain(rain: &str, slot: usize) -> String {
    ...
    for name in names {
        ...
        out = out.replace(
            ...
            &format!("@keyframes {renamed}{{"),
        );
        out = out.replace(
            &format!("animation:{name} "),
            &format!("animation:{renamed} "),
        );
    }
    out
}
...
pub fn compose(
    ...
    for (slot, tile) in tiles.iter().enumerate() {
        let (rain, content) = extract_widget(tile.svg)
            .ok_or_else(|| format!("tile {slot}: could not parse widget svg"))?;
...
<text x="25" y="35">hello {tag}</text>
    ...
    fn extracts_real_generated_widgets() {
        ...
        for name in [
            "cf-rating-dark.svg",
            "lc-skills-dark.svg",
            "github-visitors-dark.svg",
        ] {
            let svg = std::fs::read_to_string(format!("{dir}/{name}")).expect("read widget svg");
            extract_widget(&svg)
                .unwrap_or_else(|| panic!("{name}: failed to parse real widget output"));
        }
    }
}
