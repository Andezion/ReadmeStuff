...
async fn main() {
    ...
    render_card("github-stats",         github_stats_widget(&profile),        |w| render_github_stats(&w, Theme::Dark),        &out_dir);
    render_card("github-repos",         github_repos_widget(&profile),        |w| render_github_repos(&w, Theme::Dark),        &out_dir);
    render_card("github-contributions", github_contributions_widget(&profile), |w| render_github_contributions(&w, Theme::Dark), &out_dir);
    render_card("github-social",        github_social_widget(&profile),       |w| render_github_social(&w, Theme::Dark),       &out_dir);
    render_card("github-heatmap",       github_heatmap_widget(&profile),      |w| render_github_heatmap(&w, Theme::Dark),      &out_dir);
    render_card("github-monthly",       github_monthly_widget(&profile),      |w| render_github_monthly(&w, Theme::Dark),      &out_dir);
    render_card("streak",               streak_widget(&profile),              |w| render_streak(&w, Theme::Dark),              &out_dir);
    render_card("langs",                langs_widget(&profile, 6),            |w| render_langs(&w, Theme::Dark),               &out_dir);

    render_card("cf-rating", cf_rating_widget(&profile), |w| render_cf_rating(&w, Theme::Dark), &out_dir);
    render_card("cf-stats",  cf_stats_widget(&profile),  |w| render_cf_stats(&w, Theme::Dark),  &out_dir);

    render_card("cw-rank",      cw_rank_widget(&profile),      |w| render_cw_rank(&w, Theme::Dark),      &out_dir);
    render_card("cw-kata",      cw_kata_widget(&profile),      |w| render_cw_kata(&w, Theme::Dark),      &out_dir);
    render_card("cw-languages", cw_languages_widget(&profile), |w| render_cw_languages(&w, Theme::Dark), &out_dir);

    render_card("lc-solved",    lc_solved_widget(&profile),    |w| render_lc_solved(&w, Theme::Dark),    &out_dir);
    render_card("lc-skills",    lc_skills_widget(&profile),    |w| render_lc_skills(&w, Theme::Dark),    &out_dir);
    render_card("lc-languages", lc_languages_widget(&profile), |w| render_lc_languages(&w, Theme::Dark), &out_dir);
